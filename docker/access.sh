#!/usr/bin/env bash
# Print copy-paste-ready access details for the Jump Hippo sandbox. Sourced values
...
cat <<EOF
  ...
  $(bold "Jump Hippo sandbox — access details")
EOF
rule
cat <<EOF
...
cat <<EOF
  ...
  Jump Hippo tunnel (Forwarding type: Dynamic):
  ...
  Jump Hippo tunnel (Forwarding type: Remote):
  ...
  → 1) run the local target:   make sandbox-host-echo
     2) arm the tunnel, then trigger it from inside the jump:
        docker exec jumphippo-jump sh -c \\
...
rule
cat <<EOF
  Sanity-check WITHOUT Jump Hippo (proves the topology; needs the generated key):
  ...
  Host keys are regenerated on destroy→create, so Jump Hippo will show a
