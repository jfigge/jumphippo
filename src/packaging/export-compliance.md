# Port Hippo — Encryption / Export-Compliance Summary

**Product:** Port Hippo (`com.porthippo.app`)
...
> function Port Hippo contains, written to support U.S. export-compliance
...
- Port Hippo's **primary function is a networking client** — it establishes SSH
...
- The strongest data-confidentiality primitive Port Hippo invokes directly is
  **AES-256-GCM**, used only for local at-rest protection of user-entered secrets;
  ...
  computed by the bundled SSH library / OS crypto, not implemented by Port Hippo.
...
Port Hippo encrypts exactly one class of data: **saved SSH passwords and key
...
Port Hippo does **not** implement any SSH cipher itself; it uses the standard
...
### 2.4 What Port Hippo does *not* do
...
Port Hippo is distributed as a signed desktop application for macOS, Windows, and
Linux, downloadable at no charge from `porthippo.com` and GitHub releases. It is
