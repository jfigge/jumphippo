...
and not an impostor intercepting the connection. Jump Hippo verifies the host key
...
- **Known and matching** → the connection proceeds silently. This is the normal
  case.
- **Unknown** (never seen this host) → Jump Hippo prompts you to trust it — see
  Trust on first use below.
- **Changed** (known host, *different* key) → Jump Hippo **refuses** the
  connection and warns you. It never auto-accepts a changed key.

Jump Hippo **never** connects to a server it can't verify without asking you
first.
...
- **Trust** — Jump Hippo records the fingerprint in its accepted-keys store and
...
If a host you've already trusted presents a **different** key, Jump Hippo stops and
...
Jump Hippo can't tell these apart, so it **refuses to connect** and leaves the
...
**Settings → Host Keys** lists the fingerprints Jump Hippo verifies against, split
across two tabs:

- **Jump Hippo** — the keys you accepted through Jump Hippo (its own accepted-keys
...
- **Operating System** — the keys already in your `~/.ssh/known_hosts`. Jump Hippo
...
Jump Hippo carries your SSH credentials to these servers. If it authenticated to an
