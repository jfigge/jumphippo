...
Jump Hippo binds local ports and holds SSH credentials, so its security posture
...
destination it forwards to — with no further authentication at the Jump Hippo
...
Jump Hippo verifies the host key of **every** hop against your `known_hosts` and
...
The only secrets Jump Hippo stores are **SSH passwords and key passphrases**. They
...
> An **SSH agent** credential stores *nothing* in Jump Hippo — the agent holds the
> keys. It's the option that keeps the least secret material on disk.

## Choosing an auth method

- **SSH agent** — nothing sensitive stored in Jump Hippo; the agent signs. Best
  when you already run one.
- **Private key + passphrase** — Jump Hippo holds the passphrase (encrypted); the
...
the far end) — Jump Hippo **never runs a command on a remote host**, and it only
...
## Jump Hippo never phones home

Jump Hippo makes **no analytics, telemetry, or tracking calls**. It talks to the
...
machine. It's open source ([GitHub](https://github.com/jfigge/jumphippo#readme)),
...
- Use the **OS keychain** or a **master password** on a shared or portable
  machine.
- Keep your OS and Jump Hippo updated (**Help → Check for Updates**; builds
  installed from the Mac App Store or Microsoft Store update through the store
  instead, so the menu item is absent there).
