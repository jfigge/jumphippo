...
`address:port`. If Jump Hippo reports the port is in use:

- Another app (or another Jump Hippo tunnel) already holds it. Pick a different
  **entry port**, or free the one that's taken.
- Jump Hippo tells you *which* of your own tunnels owns a conflicting port when it
  can.
...
- Prefer a **high port** (≥ 1024) as the entry port and point your client at it —
  simplest and safest.
- Or run Jump Hippo with the privileges needed to bind low ports (not
...
Jump Hippo **refuses** a connection when a host's key differs from the one you
...
- With **auto-reconnect off** (the default), Jump Hippo returns the tunnel to
...
finally expires (which can be minutes). Jump Hippo's **SSH keepalive** sends a small
...
Jump Hippo can raise a desktop notification when a tunnel **drops**, **recovers**,
...
If you use the **master password** backend, Jump Hippo starts locked and can't
...
[GitHub issue](https://github.com/jfigge/jumphippo/issues) — secrets are stripped.
