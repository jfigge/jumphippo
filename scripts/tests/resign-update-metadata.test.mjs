...
test("recomputes sha512/size for every referenced file and the top-level path", () => {
  ...
  try {
    ...
    assert.deepEqual(valuesOf(out, "sha512"), [
      sha512(x64),
      sha512(arm),
      sha512(x64),
    ]);
    assert.deepEqual(valuesOf(out, "size"), [
      String(x64.length),
      String(arm.length),
    ]);
    ...
    assert.ok(
      !/blockMapSize/.test(out),
      "blockMapSize lines should be removed",
    );
...
test("deletes stale *.blockmap sidecars", () => {
  ...
  try {
    ...
    writeFileSync(
      join(dir, "App-Setup-1.0.0-x64.exe.blockmap"),
      Buffer.from("stale"),
    );
    writeFileSync(
      join(dir, "latest.yml"),
